...
export default function CreateEventPage() {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      data.append(
        'bulk_pass_price',
        formData.bulk_pass_price
      );

      data.append(
        'bulk_pass_entries',
        formData.bulk_pass_entries
      );
                ...
                <div className="bg-black/20 p-6 rounded-3xl border border-cyan-500/30">
                  ...
                  <div className="grid md:grid-cols-3 gap-5 mb-8 bg-black/40 p-5 rounded-2xl border border-white/5">
                    <div>
                      ...
                      <input type="datetime-local" min={getMinDateTime()} name="slab1_deadline" value={formData.slab1_deadline} onChange={handleChange} className="w-full p-3 rounded-xl bg-black/60 border border-white/10 text-white" />
                    </div>
                    <div>
                      <label className="block text-gray-400 text-sm mb-2">Slab 2 Deadline</label>
                      <input type="datetime-local" min={getMinDateTime()} name="slab2_deadline" value={formData.slab2_deadline} onChange={handleChange} className="w-full p-3 rounded-xl bg-black/60 border border-white/10 text-white" />
                    </div>
                    <div>
                      <label className="block text-gray-400 text-sm mb-2">Slab 3 Deadline</label>
                      <input type="datetime-local" min={getMinDateTime()} name="slab3_deadline" value={formData.slab3_deadline} onChange={handleChange} className="w-full p-3 rounded-xl bg-black/60 border border-white/10 text-white" />
                    </div>
                  </div>
